...
25. **One Cycle Per Session rule — each SDD cycle must run in a dedicated chat session** (proposed 2026-03-21, pending implementation via orchestrator-mandatory-new-session) — User decision: to prevent context bleed, orchestrator confusion from prior-phase artifacts, and degraded sub-agent focus, each `/sdd-ff` or `/sdd-new` cycle must be started in a fresh chat session. The orchestrator will enforce this by detecting an active in-progress cycle and blocking the start of a new one in the same session. Companion proposals also address orchestrator teaching personality, scope estimation signaling, natural language comprehension gaps, and action control gate UX. Proposals at `openspec/changes/2026-03-21-orchestrator-mandatory-new-session/`, `openspec/changes/2026-03-21-orchestrator-teaching-personality/`, `openspec/changes/2026-03-21-orchestrator-scope-estimation/`, `openspec/changes/2026-03-21-orchestrator-natural-language/`, `openspec/changes/2026-03-21-orchestrator-action-control-gates/`.
